...
ozone_var <- "mean_avg_daily_peak_ppm"  # <- change here for sensitivity
...
mort_env_mod <- mort_env |>
  mutate(
    ...
    temp_c = scale(daily_mean, center = TRUE, scale = FALSE)[, 1],
...
m_qp <- glm(
  Deaths ~ ozone_10ppb_c + sex + age_group + temp_c + poly(year,3) +
...
# RR = 1.0105 (95% CI: 0.9740, 1.0484)
# A positive association (RR>1) between annual death counts and the annual mean
# of the daily 1-h peak ozone. However, the 95% CI contains 1 and p-value > 0.05,
# so the association is not statistically significant.
...
summary(m_qp_sex) # note the p-value for ozone_10ppb_c:sexMale is <0.05,
...
################################################################################
### By age (10 age groups)
m_qp_age10 <- glm(
  Deaths ~ ozone_10ppb_c * age_group + sex + temp_c + poly(year,3) +
    offset(offset_log_pop),
  family = quasipoisson(link = "log"),
  data = mort_env_mod
)
...
coefs <- coef(m_qp_age10)     # <- your quasi-Poisson model object
vc    <- vcov(m_qp_age10)

base_var <- "ozone_10ppb_c"   # slope name for reference age group
age_fac  <- "age_group"     # factor name used in the model
levels10 <- c("0","1-4","5-14","15-24","25-44",
              "45-64","65-74","75-84","85-94","95+")
...
# Age group 45-64, 85-94, and 95+ have statistically significant RRs >1 
# Age group 0 and 75-84 have statistically significant RRs <1 
# Show RR and 95% CI in a plot
library(dplyr)
library(ggplot2)

# keep the current data-frame order
age_order <- age_rr10$age_group

age_rr10 <- age_rr10 %>%
  mutate(age_group = factor(age_group, levels = age_order))

age_plot <- ggplot(age_rr10, aes(x = age_group, y = RR)) +
  geom_point(size = 2.5) +
...
# The plot shows that mortality risk associated with annual ozone
# tends to increase with age

# Linear trend test
mort_env_mod <- mort_env_mod %>%
  mutate(age_ord = as.numeric(factor(age_group, 
                                     levels = c("0","1-4","5-14","15-24",
                                                "25-44","45-64","65-74",
                                                "75-84","85-94","95+"))))
...
# However, linear trend test shows a p-value 0.745 >0.05, providing no evidence
# to support a linear trend.
